...
try:
    ...
    print(f"OOM: Cannot load {args.model} on {gpu_name} — skipping")
    exit(0)
...
try:
    ...
    print(f"OOM during warmup at batch_size={args.batch_size} — skipping this configuration")
    exit(0)
...
for i in range(args.num_batches):
    try:
        ...
        print(f"OOM at batch {i}, batch_size={args.batch_size} — skipping remaining batches")
        break

# ============================================================
# STATISTICS (only if we have results)
# ============================================================
if len(results) == 0:
    print("No successful batches — exiting")
    exit(0)
...
print(f"  Completed batches: {len(results)}/{args.num_batches}")
...
csv_file = os.path.join(
    args.output_dir,
    f"bench_{args.model}_bs{args.batch_size}_{args.optimizer}_{gpu_name_clean}_{timestamp}.csv"
)
...
summary_file = os.path.join(
    args.output_dir,
    f"summary_{args.model}_bs{args.batch_size}_{args.optimizer}_{gpu_name_clean}_{timestamp}.csv"
)
with open(summary_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["model", "batch_size", "optimizer", "gpu", "gpu_mem_gb", "num_batches_completed",
                     "mean_ms", "std_ms", "median_ms", "min_ms", "max_ms", "throughput_img_s"])
    writer.writerow([args.model, args.batch_size, args.optimizer, gpu_name, f"{gpu_mem:.1f}", len(results),
                     f"{mean_time*1000:.2f}", f"{std_time*1000:.2f}", f"{median_time*1000:.2f}",
                     f"{min_time*1000:.2f}", f"{max_time*1000:.2f}", f"{args.batch_size/mean_time:.1f}"])
